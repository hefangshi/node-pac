...
function NpmStrategy(options) {
  ...
  this.modulePath = Path.join(this.cwd, 'packed_modules');
...
NpmStrategy.prototype._install = function (type, destPath, callback) {
  ...
  async.eachSeries(glob.sync('*.tgz', {
    ...
    log('\n');
    _.difference(Object.keys(deps), Object.keys(extracted)).forEach(function (name) {
      var fv = name;
      if (self.verbose) {
        error('ERROR: Module', fv, 'is specified in the package.json, but not in tgz');
      }
    });
    log('Done!');
    if (callback) {
...
NpmStrategy.prototype._packAll = function (srcList, curInst, callback) {
  ...
  _.difference(Object.keys(srcList), Object.keys(curInst)).forEach(function (name) {
    error('ERROR:', name, 'is not installed!');
  });
